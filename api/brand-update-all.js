export default async function handler(req, res) {
  try {
    ...
    if (!Array.isArray(brands) || brands.length === 0) {
      return res.status(400).json({ error: "Missing brands array" });
    }
...
You are building a washing machine technical database across multiple brands.

Rules:
- Return ONLY structured JSON matching the schema.
- Preserve the same JSON structure exactly.
- Expand and improve each selected brand deeply.
- Avoid duplicate models within each brand.
...
`;
    ...
    const payloadForModel = brands.map((b) => ({
      id: b?.brand?.id || b?.id || "",
      name_en: b?.brand?.name_en || b?.name_en || "",
      name_ar: b?.brand?.name_ar || b?.name_ar || "",
      logo: b?.brand?.logo || b?.logo || "",
      current_models_count: Array.isArray(b?.models) ? b.models.length : 0,
      current_data: b
    }));

    const userMessage = `
Update all these brands:
...
${
  instruction ||
  "Expand all brands with older and newer models, fuller fault families, cleaner aliases, better board mapping notes, and more complete parts while preserving the same JSON structure and avoiding duplicates."
}
`;
